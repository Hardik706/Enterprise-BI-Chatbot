...
from core.db_agent import DBAgentManager
...
if "db_manager" not in st.session_state:
    st.session_state.db_manager = DBAgentManager()
...
def reconnect_db():
    # Update settings from session input
    settings.DB_HOST = st.session_state.db_host
    settings.DB_PORT = st.session_state.db_port
    settings.DB_USER = st.session_state.db_user
    settings.DB_PASSWORD = st.session_state.db_pass
    settings.DB_NAME = st.session_state.db_name
    
    st.session_state.db_manager = DBAgentManager()
    st.toast("Reconnecting to MySQL database...", icon="🔄")
...
with col_status1:
    if db_manager.db and not db_manager.connection_error:
        # Fetch list of active tables
        try:
            from sqlalchemy import inspect
            tables = inspect(db_manager.db._engine).get_table_names()
        ...
        err_msg = db_manager.connection_error or "Configure connection parameters in the sidebar."
        st.markdown(
        ...
        try:
            ...
            if routing.route == "sql":
                with st.spinner("SQL Agent searching database..."):
                    sql_res = db_manager.ask_agent(cleaned_input)
